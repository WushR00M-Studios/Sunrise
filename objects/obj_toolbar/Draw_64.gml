...
if os_type == os_windows {
	...
	draw_text(1344, 16, "ALPHA functionality, it's subject to change!");
...
//} else if os_type == os_android {
	...
	// Draw full black background across top
	draw_set_color(c_black);
	draw_rectangle(0, 0, screen_w, toolbar_h + 16, false); // filled background
	...
	for (var i = 0; i < array_length(buttons); i++) {
	    ...
	    // Button background
	    draw_set_color(button_color);
	    draw_rectangle(bx, by, bx + button_width, by + toolbar_h + 16, false);
	    ...
	    draw_set_color(c_white);
	    var label_text = buttons[i].name;
	    draw_text(bx + button_width / 2, by + (toolbar_h + 16) / 2, label_text);
	}
	...
	// Toolbar border line at bottom of toolbar
	draw_set_color(make_color_rgb(35, 35, 35)); // or c_gray for softer line
	draw_line(0, toolbar_h, screen_w, toolbar_h + 16);
}