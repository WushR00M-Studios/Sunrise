...
// Gamepad Support
} else if !global.inputtype {
    if InputPressed(INPUT_VERB.RIGHT) or InputPressed(INPUT_VERB.JOY_RIGHT) {
        selected_index = clamp(selected_index + 1, 0, array_length(global.accounts) - 1);
    }
    if InputPressed(INPUT_VERB.LEFT) or InputPressed(INPUT_VERB.JOY_LEFT) {
        selected_index = clamp(selected_index - 1, 0, array_length(global.accounts) - 1);
    }
