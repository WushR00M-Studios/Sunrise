	...
	// === JUMPING ===
	if (on_ground && InputCheck(INPUT_VERB.ACCEPT)) {
	    vsp = jump_strength;
	    on_ground = false;
	    double_jump = true;
	} else if (!on_ground && double_jump && InputCheck(INPUT_VERB.ACCEPT)) {
	    vsp = jump_strength;
	    double_jump = false;
	}

	// === HOMING ATTACK ===
	if ((!on_ground && InputPressed(INPUT_VERB.ACCEPT)) && can_homing) {
	    var nearest = noone;
	...
	// === MELEE ===
	if ((!on_ground && InputCheck(INPUT_VERB.CANCEL)) && can_homing) {
	    var nearest = noone;
	    var best_dist = homing_range;
